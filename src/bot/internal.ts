  ...
  async getGuild(guildId: string): Promise<Universal.Guild>
  {
    try
    {
      ...
      return {
        ...
        avatar: getMediaProxyUrl(_payload.data.group.avatarUrl, 'image', this.bot)
      };
    } catch (error)
    {
    ...
    try
    {
      ...
      if (userPayload.data?.user?.userId)
      {
        return {
          ...
          avatar: getMediaProxyUrl(userPayload.data.user.avatarUrl, 'image', this.bot),
      ...
      try
      {
        ...
        if (botPayload.data?.bot?.botId)
        {
          return {
            ...
            avatar: getMediaProxyUrl(botPayload.data.bot.avatarUrl, 'image', this.bot),
            isBot: true, // 这是一个机器人
          };
        }
      } catch (botError)
