...
> 官方文档：<https://www.yhchat.com/document/400-452>
...
> 即：注意上传网络图片的时候，如果 URL 里不带后缀名，需要处理后缀名后再上传。
...
当 MIME 类型为 `image/jpeg` 时，文件扩展名应为 `.jpg`，不要使用 `.jpeg`。
:::
...
上传成功后返回的 `imageKey` 不是最终访问地址，不能直接拿来拼接图片 URL。`imageKey` 只是上传成功后的返回值，真正的访问地址需要根据图片内容重新计算哈希。

图片访问地址的生成规则如下：

`https://chat-img.jwznb.com/{imageHash}.jpg`

其中 `imageHash` 是对图片文件内容计算得到的哈希值，不是接口返回的 `imageKey`。

例如，上文我们上传的是 `D:/Pictures/素材图片/7.jpg`，那么在代码里会先读取这个文件内容，再对文件内容做哈希计算，得到：

`c91bb351c5fc283dfd9c95d0ec5d6c88`

然后把这个哈希值拼接到云湖图片资源地址后面：

`https://chat-img.jwznb.com/c91bb351c5fc283dfd9c95d0ec5d6c88.jpg`

具体代码参考：

- [ImageUploader.ts](https://github.com/FLY1919/adapter-yunhupro/blob/main/src/uploader/ImageUploader.ts)
